...
| 3   | HTTP JSON vertical slice — first real protocol path through Worker V2                     | in progress   | [`arcs/worker-v2-build.md`](./arcs/worker-v2-build.md)           | [Worker V2 plan — Phase 2](../../component-architecture/worker-v2/README.md#phase-2-http-json-vertical-slice)     |
...
PR 4 — Legacy compatibility adapter: connects Worker V2 to the current engine unchanged (translate the current queued job event into `ExecuteJobCommand`, invoke Worker V2, translate `JobResult` back into the current completion/failure compatibility event). Not yet discussed. Starting reference: [Worker V2 plan — Phase 3](../../component-architecture/worker-v2/README.md#phase-3-legacy-compatibility-adapter).

PRs 5–6 continue the same plan's phased sequence (the engine's migration to a `WorkerDispatch` outbound port; then completing the worker surface — MCP, binary capture, removing the old worker/tool-registry/router). All seeded from [`docs/component-architecture/worker-v2/README.md`](../../component-architecture/worker-v2/README.md)'s phased plan, itself synthesized from PR 1's investigation plus a larger-model review (see that milestone's own arc file for how the plan was produced) — a starting reference, not a commitment. Each PR still gets discussed on its own terms before being planned, per the usual workflow, and may end up split further once that discussion happens.
