# Architecture Decision Records

One file per significant, settled decision — see [ADR-0001](0001-adopt-architecture-decision-records.md) for the convention itself (location, numbering, template, and the immutability/supersession rule).

| #                                                   | Title                                                  | Status   |
| --------------------------------------------------- | ------------------------------------------------------ | -------- |
| [0001](0001-adopt-architecture-decision-records.md) | Adopt Architecture Decision Records                    | Accepted |
| [0002](0002-artifact-flow-association-schema.md)    | Artifact-to-flow association schema for Artifacts mode | Accepted |
