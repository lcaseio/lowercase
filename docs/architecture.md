...
| **`replay`**                  | Raw event history (JSONL log), separate from the SQL metadata store. Its exact tier is still an open question (adapter vs. something closer to Operations/application services) — see the [`architecture-boundaries` milestone](./milestones/architecture-boundaries/MILESTONE.md)'s "Not yet scoped" section. |
...
- **The evidence and discussion behind a decision**: `docs/milestones/architecture-boundaries/research/` and its `arcs/` — working notes, not meant to be tidy.
