...
export class LegacyFsArtifactStore implements LegacyArtifactStorePort {
  baseDir: string;
  rootPath: string;
  ...
  async findAbsoluteFilePath(hash: string): Promise<string | null> {
    ...
    // Both stores are wired to the same root directory during the
    // legacy/new transition (see ArtifactWriter/FsArtifactStore). The new
    // store writes content with no extension at all, so a hash it produced
    // is otherwise invisible to this probe -- check that bare path too,
    // temporarily, until reads migrate off this store.
    const bareFilePath = path.join(
      this.getAbsoluteDirPath(hash),
      hash.slice(4),
    );
    if (await this.exists(bareFilePath)) return bareFilePath;
